...
public class AdminManageBuildingViewController {
    ...
    private void initialize() {
        try {
            ...
            buildingRoomCountColumn.setCellValueFactory(cell -> new SimpleStringProperty(
                    String.valueOf(cell.getValue().getBuildingRoom_count().get())));
        ...
        try {
            if (selectedIndex >= 0) {
                ...
                BuildingServerCommunication.updateBuilding(selectedBuilding.getBuildingId().get(),
                        tempBuilding.getBuildingName().get(), tempBuilding.getBuildingRoom_count().get(),
